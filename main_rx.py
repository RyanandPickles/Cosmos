...
rx = PlutoReceiver()
rx.set_sdr(sdr_rx)
rx.set_sample_rate(6_000_000)
rx.set_buffer_size(500e3)
rx.set_channel(7)
rx.set_gain_level(80)
rx.desired_transmit_symbols_real = False  # genuinely complex 16-QAM, don't discard Q
rx.num_transmit_symbols = 8192            # matches main_tx.py's num_qam_symbols
...
if True:
    ...
    plt.grid(True)
    plt.legend()
    filename = dir_plots + 'main_tx_rx_qam' + '.pdf'
    plt.savefig(filename)
    filename = dir_plots + 'main_tx_rx_qam' + '.svg'
    plt.savefig(filename)
    plt.show()
